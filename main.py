...
def prepare_data(ppt_assistant, args):
    instructions, labels = dataset.load_data(
        args.user_path + args.data_path, args.dataset, args
    )
    ...
    for idx, dialogue in enumerate(instructions):
        ...
        if args.dataset == "long":
            ppt_assistant.load_ppt(
                os.path.join(args.user_path + "long_slides", f"{idx}.pptx")
            )
        else:
            ppt_assistant.load_ppt(None)

        set_name = (
            "Edit_PPT_template" if args.dataset == "long" else "Create_new_slides"
        )
        ...
        for step, instruction in enumerate(dialogue):
            ...
            if args.api_lack:
                ppt_executor.save_ppt(
                    args.user_path
                    + f"PPT_Base_File/{set_name}_API_lack/{idx}_{step}.pptx"
                )
            else:
                ppt_executor.save_ppt(
                    args.user_path + f"PPT_Base_File/{set_name}/{idx}_{step}.pptx"
                )
            ...
            if args.api_lack:
                ppt_executor.save_ppt(
                    args.user_path
                    + f"PPT_Label_File/{set_name}_API_lack/{idx}_{step}.pptx"
                )
            else:
                ppt_executor.save_ppt(
                    args.user_path + f"PPT_Label_File/{set_name}/{idx}_{step}.pptx"
                )
...
def test(ppt_assistant, args):
    ...
    for sess_id, session_path in enumerate(
        utils.sorted_list(args.user_path + f"PPT_test_input/{set_name}")
    ):
        session = utils.parse_train_json(
            args.user_path + f"PPT_test_input/{set_name}/{session_path}"
        )
        chat_history = []
        for turn_id, turn in tqdm(enumerate(session)):
            ...
            if args.resume:
                ...
                if args.tf and os.path.exists(
                    args.user_path
                    + f"PPT_Pred_File/{set_name}/{args.exp_name}_{sess_id}_{turn_id}.pptx"
                ):

                    print("Exists!")
                    continue
                if args.sess and os.path.exists(
                    args.user_path
                    + f"PPT_Pred_File/{set_name}/{args.exp_name}_{sess_id}_{len(session)-1}.pptx"
                ):
                    print("Exists!")
                    continue
            (
                ...
                base_ppt_path,
                label_ppt_path,
                api_lack_base_ppt_path,
                api_lack_label_ppt_path,
            ) = turn
            ...
            if args.tf:
                if args.api_lack:
                    ...
                    ppt_assistant.load_ppt(args.user_path + base_ppt_path)
                    label_file = label_ppt_path
                ppt_assistant.load_chat_history(
                    [x[0] for x in chat_history],
                    [x[1].strip(";").split(";") for x in chat_history],
                )
                prompt, reply = ppt_assistant.chat(
                    splitted_instruction,
                    ppt_path=args.user_path + base_ppt_path,
                    verbose=False,
                )
                ...
                utils.write_lines(
                    [prompt],
                    args.user_path
                    + f"PPT_Prompt_File/{set_name}/{args.exp_name}_{sess_id}_{turn_id}.txt",
                )
                # import pdb
                # pdb.set_trace()
                ...
                with jsonlines.open(
                    args.user_path
                    + f"PPT_test_output/{set_name}/{args.exp_name}_session_{sess_id}.json",
                    mode="a",
                ) as writer:
                    data = {
                        "Turn": turn_id,
                        ...
                        "Pred File": f"PPT_Pred_File/{set_name}/{args.exp_name}_{sess_id}_{turn_id}.pptx",
                        "Label File": label_file,
                        "Prompt File": f"PPT_Prompt_File/{set_name}/{args.exp_name}_{sess_id}_{turn_id}.txt",
                    }
                    writer.write(data)
                ...
                prompt, reply = ppt_assistant.chat(
                    instruction, ppt_path=None, verbose=False
                )
                apis = utils.parse_api(reply)
                ppt_assistant.api_executor(apis, test=True)
    ...
    for idx, dialogue in tqdm(enumerate(instructions)):
        for step, instruction in enumerate(dialogue):
            ...
            for xx in label_apis:
                if (
                    ("align_slide" in xx.split("(")[0])
                    or (
                        xx.split("(")[0]
                        in ["set_left", "set_right", "set_top", "set_bottom"]
                    )
                    or ("corner" in xx.split("(")[0])
                ):
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", default="short", type=str, help="short/long")
    parser.add_argument("--model_id", default="None", type=str, help="short/long")
    parser.add_argument(
        "--user_path", default="./PPTC/", type=str, help="the user storage file path "
    )
    parser.add_argument(
        "--save_path",
        default="test_pptx_data",
        type=str,
    ...
    # modeling
    parser.add_argument("--model", default="turbo", type=str, help="turbo/gpt4/text3")
    parser.add_argument(
        "--planning",
        default=False,
        action="store_true",
    ...
    parser.add_argument(
        "--second", default=False, action="store_true", help="second test"
    )
    ...
    parser.add_argument("--robust_num", default=0, type=int, help="which robusted data")
    parser.add_argument(
        "--noisy",
        default=False,
        action="store_true",
